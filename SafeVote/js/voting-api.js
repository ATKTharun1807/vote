...
// --- FIREBASE CONFIG ---
// Handle potential environment variables safely
...
const firebaseConfig = rawConfig ? (typeof rawConfig === 'string' ? JSON.parse(rawConfig) : rawConfig) : { apiKey: "placeholder" };
const firebaseApp = initializeApp(firebaseConfig);
const auth = getAuth(firebaseApp);
const db = getFirestore(firebaseApp);
const appId = getGlobal('__app_id', 'voting-ocean');
...
export class VotingAPI {
    ...
    async initAuth() {
        if (!auth) return;
        try {
            const initialToken = getGlobal('__initial_auth_token');
